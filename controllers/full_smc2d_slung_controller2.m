function [u,s] = full_smc2d_slung_controller2(q,qdot,xref,physics_p,control_p)
...
theta_c_lim = 0.6;
theta_c = -lambda_alpha*alpha;
theta_c = min(theta_c_lim, max(-theta_c_lim, theta_c));
thetadot_c = 0;


K1 = control_p.K1; K2 = control_p.K2; K3 = control_p.K3;

% lambda_thetadot = lambda_x*m*U1*sin(2*alpha)/(K3*2*M*(M+m));
u1n = zddot_d-fz+lambda_z*(zdot_d-zdot);
lambda_thetadot = lambda_x*M*u1n*(M+m)/(K3*(M+m*sin(theta)^2)^2);
lambda_theta = lambda_thetadot*K1;
% lambda_thetadot = -sign(sin(alpha))*lambda_thetadot;
% lambda_theta = -sign(sin(alpha))*lambda_theta;
...
u = [U1;U2];
s = [s1(:); s2(:); theta_c(:); lambda_thetadot(:); lambda_theta(:)];
